...
YELP_CLIENT_ID = getattr(settings, 'YELP_CLIENT_ID', 'b6xCcdMWdVy4wehyey_jrw')
YELP_CLIENT_SECRET = getattr(settings, 'YELP_CLIENT_SECRET','cRS7pf14yl9h2wTYS6Oo7sSanVFl91zWjA5uP5PKb3DkavNZneKjKwzjHqKKN9JJ')
...
def get_token(request=None):
    ...
    if not token_exists:
        ...
        if request:
            ...
            expires_in_tstamp = expires_in.fromtimestamp(time.time())
